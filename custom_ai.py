...
class AIMessage(BaseMessage):
    ...
    @classmethod
    def from_parsed_message(cls, parsed_message: Dict[str, Any]) -> "AIMessage":
        """Create an AIMessage from a parsed message dictionary.

        Args:
            parsed_message: Dictionary containing parsed message data

        Returns:
            AIMessage: New instance with parsed data
        """
        # Extract content string - join text blocks if multiple exist
        content = "\n".join(
            block["text"] for block in parsed_message.get("content", [])
            if block["type"] == "text"
        )
        ...
        return cls(
            ...
            response_metadata={
                "model": parsed_message.get("metadata", {}).get("model"),
                "stop_reason": parsed_message.get("metadata", {}).get("stop_reason"),
                "usage": parsed_message.get("metadata", {}).get("usage", {})
            },
            content_blocks=parsed_message.get("content", []),
            code_blocks=parsed_message.get("code_blocks", []),
